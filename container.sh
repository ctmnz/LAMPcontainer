...
yum --installroot=$container_dir install -y memcached
...
# M (mysql)
yum --installroot=$container_dir install -y mariadb mariadb-server memcached


# P (php)
echo "Installing php modules.... "
phpmodules=`cat ./P/php.modules |grep -v ^# |awk {'print $1'} | cut -d'.' -f1`
for module in $phpmodules; do
 echo "Installing $module ..."
 yum --installroot=$container_dir install -y $module
done
yum --installroot=$container_dir install -y phpMyAdmin
# =============================================
# postinstalling operations
